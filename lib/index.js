...
// Pro tip: look up octokit.rest.* methods by using the operationId listed in
...
async function getLastSuccessfulCheckRun (octokit, repo) {
  const checkRunsResp = await octokit.rest.checks.listForRef({
    owner: repo.owner.login,
    repo: repo.name,
    ref: repo.default_branch
  })
  const successfulCheckRuns = checkRunsResp.data.check_runs
    .filter(s => s.conclusion === 'success')
  if (successfulCheckRuns.length === 0) {
    return null
  }

  return successfulCheckRuns.reduce((max, current) => {
    return new Date(current.started_at) > new Date(max.started_at) ? current : max
  }, successfulCheckRuns[0]).started_at
}

export async function syncToProject (org, projectNumber) {
  ...
  const fields = await octokit.paginate(
    ...
    { org, project_number: projectNumber }
  )
  ...
  const nonArchivedRepos = repos
    ...
    .toSorted((repoA, repoB) => repoA.name.localeCompare(repoB.name))
  ...
  for (const repo of nonArchivedRepos) {
    ...
    try {
      lastSuccessfulCheckRun = await getLastSuccessfulCheckRun(octokit, repo)
      if (lastSuccessfulCheckRun) {
        console.log(`  Last successful check run: ${lastSuccessfulCheckRun}`)
      } else {
